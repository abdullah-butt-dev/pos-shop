...
export default function ReceivablesPage() {
  ...
  const [customerField, setCustomerField] = useState<AutocompleteOption | null>(
    null,
  );
  ...
  const customersWithBalance = customerSummaries.filter(
    (c) => c.outstanding > 0,
  ).length;
  ...
  return (
    <main className="h-full w-full flex flex-col overflow-hidden bg-[var(--pos-panel-2)] text-foreground">
        ...
        <div className="max-w-5xl mx-auto px-4 py-5 space-y-5">
          ...
          <div className="pos-panel rounded-xl p-6 flex flex-col items-center justify-center text-center space-y-2 border border-[var(--pos-stroke)]">
            ...
            <p className="text-sm text-muted-foreground">
              From {customersWithBalance} customer
              {customersWithBalance === 1 ? "" : "s"} with outstanding balances
            </p>
          </div>
            ...
            <div className="pos-panel rounded-lg p-4 flex flex-col gap-3 w-full lg:w-80 shrink-0">
              ...
              {salesLoading ? (
                ...
                <p className="text-sm text-muted-foreground flex items-center gap-1.5">
                  <UserPlus className="w-3.5 h-3.5" /> No sales recorded yet —
                  search above to add a customer.
                </p>
              ) : (
                ...
                <>
                  <div className="pos-panel rounded-lg p-4 grid grid-cols-3 gap-4">
                    <div>
                      ...
                      <p className="text-base font-bold">
                        {formatMoney(selectedSummary.totalSold)}
                      </p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">
                    ...
                    <div>
                      ...
                      <p
                        className={cn(
                          "text-base font-bold",
                          selectedSummary.outstanding > 0.009
                            ? "text-amber-500"
                            : "text-emerald-500",
                        )}
                      >
                  ...
                  <div className="pos-panel rounded-lg p-4 flex flex-col gap-3">
                    ...
                    {customerSales.length === 0 ? (
                      <p className="text-sm text-muted-foreground">
                        No sales recorded for this customer yet. Sales are
                            ...
                            {payments.map((pay) => (
                              ...
                              >
                                ...
                                <td className="py-2 pr-3 whitespace-nowrap text-muted-foreground">
                                  {pay.pos_sales
                                    ? formatDate(pay.pos_sales.sale_date)
                                    : "—"}
                                </td>
                                <td className="py-2 pr-3 text-right whitespace-nowrap font-semibold">
      ...
      <AlertDialog
        ...
        onOpenChange={(open) => !open && setPaymentToConfirm(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Confirm Payment</AlertDialogTitle>
            <AlertDialogDescription>
              Record payment of Rs {paymentToConfirm?.amount} from{" "}
