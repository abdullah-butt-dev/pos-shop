...
export async function POST(request: Request) {
  try {
    ...
    if (saleError) {
      console.error(
        "[API /api/pos/customer-payments] Sale lookup error:",
        saleError,
      );
...
// PATCH /api/pos/customer-payments -> edits a recorded customer payment
export async function PATCH(request: Request) {
  try {
    const body = await request.json();
    const { payment_id, amount, payment_date } = body || {};
    ...
    if (typeof payment_date === "string" && payment_date.trim())
      updates.payment_date = payment_date.trim();

    if (Object.keys(updates).length === 0) {
      return NextResponse.json(
        { error: "Provide amount (>0) or payment_date to update" },
        { status: 400 },
      );
    }
    ...
    if (error) {
      console.error(
        "[API /api/pos/customer-payments PATCH] Supabase error:",
        error,
      );
      return NextResponse.json({ error: error.message }, { status: 500 });
    }
    ...
    console.error(
      "[API /api/pos/customer-payments PATCH] Unexpected error:",
      err,
    );
    return NextResponse.json(
      { error: err?.message || "Internal server error" },
