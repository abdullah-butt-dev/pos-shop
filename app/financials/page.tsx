...
export default function FinancialsPage() {
  ...
  const money = (value: number) =>
    `${data?.settings?.currency || "PKR"} ${Number(value || 0).toLocaleString(
      "en-PK",
      {
  ...
  const transactions = useMemo(() => {
    const rows = [
      ...(data?.sales || []).map((x: any) => ({
        ...
        date: x.sale_date,
        type: "Sale",
        party: x.pos_customers?.name || "Walk-in Customer",
        ref: x.receipt_number,
        amount: Number(x.total_amount),
        detail: `Profit ${money(x.profit)}`,
      })),

      ...(data?.customer_payments || []).map((x: any) => ({
        id: `cp-${x.id}`,
        date: x.payment_date,
        type: "Customer Payment",
        party: x.pos_customers?.name || "Customer",
        ref: x.pos_sales?.receipt_number || "Unlinked",
        amount: Number(x.amount),
        detail: x.payment_method || "Payment",
      })),

      ...(data?.purchases || []).map((x: any) => ({
        id: `purchase-${x.id}`,
        date: x.purchase_date,
        type: "Supplier Purchase",
        party: x.pos_suppliers?.name || "Supplier",
        ref: x.reference_number || x.id.slice(0, 8),
        amount: Number(x.total_amount),
        detail: `Outstanding ${money(x.amount_due)}`,
      })),

      ...(data?.supplier_payments || []).map((x: any) => ({
        id: `sp-${x.id}`,
        date: x.payment_date,
        type: "Supplier Payment",
        party: x.pos_suppliers?.name || "Supplier",
        ref: x.pos_purchases?.reference_number || "Purchase",
        amount: Number(x.amount),
        detail: x.payment_method || "Payment",
      })),
    ];
        ...
        <div className="max-w-6xl mx-auto px-4 py-5 space-y-5">
          <header>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Historical transactions plus current outstanding balances.
            </p>
          </header>
          ...
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
            ...
            <Metric
              ...
              value={money(data?.summary.purchases)}
            />

            <Metric
              icon={CircleDollarSign}
              label="Customer Payments"
              value={money(data?.summary.customer_payments)}
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <Balance
              ...
              value={money(data?.summary.receivables)}
              href="/receivables"
            />

            <Balance
              title="Current Supplier Payables"
              value={money(data?.summary.payables)}
              href="/payables"
            />
          </div>
          ...
          <div className="pos-panel rounded-xl overflow-hidden">
            <div className="p-4 border-b border-[var(--pos-stroke)]">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Sales, payments and purchases in the selected period.
              </p>
            </div>
...
function Metric({ icon: Icon, label, value }: any) {
  return (
    <div className="pos-panel rounded-xl p-4">
      ...
      <p className="text-xl font-bold mt-1">{value || "PKR 0.00"}</p>
    </div>
  );
}

function Balance({ title, value, href }: any) {
  return (
    <a
      href={href}
    ...
    >
      ...
      <p className="text-2xl font-bold mt-2">{value || "PKR 0.00"}</p>
