...
async function run() {
  ...
  if (!functionsStatus.pos_delete_sale || !functionsStatus.pos_delete_sales_bulk || !functionsStatus.pos_delete_purchase) {
  ...
  try {
    ...
    const purchaseIds = [];
    for (let i = 1; i <= 3; i++) {
      const { data: pId, error: pErr } = await supabase.rpc("pos_create_purchase", {
      ...
      if (pErr) throw new Error(`Failed to create purchase ${i}: ` + pErr.message);
    ...
    const { data: invAfterPurchases } = await supabase
      ...
      .single();
    console.log(`Inventory after 3 purchases of 10 units each: ${invAfterPurchases?.quantity} (expected 30)`);
    ...
    const { data: invAfterSales } = await supabase
      ...
      .eq("product_id", product.id)
      .single();
    console.log(`Inventory after 2 sales: ${invAfterSales?.quantity} (expected 15)`);
    ...
    const { data: blkRes, error: blkErr } = await supabase.rpc("pos_delete_purchase", {
    ...
    if (blkErr) {
      ...
      if (!blkErr.message.includes("already been sold") && !blkErr.message.includes("blocked by sale")) {
        throw new Error("Blocking message did not describe sold items: " + blkErr.message);
      }
    } else {
      throw new Error("FAIL: Purchase with sold items was deleted when it should have been blocked!");
    }

    // TEST 2: Blocked bulk purchases deletion
    if (functionsStatus.pos_delete_purchases_bulk) {
      console.log("\n--- TEST 2: Blocked bulk purchase deletion (Batch containing sold purchase) ---");
      const { data: blkBulkRes, error: blkBulkErr } = await supabase.rpc("pos_delete_purchases_bulk", {
        p_purchase_ids: [purchaseIds[0], purchaseIds[2]], // P1 is sold, P3 is unsold
      });
      if (blkBulkErr) {
        console.log(`✓ Batch correctly blocked with error: "${blkBulkErr.message}"`);
        // Verify P3 was NOT deleted (all-or-nothing rollback)
        const { data: p3Check } = await supabase.from("pos_purchases").select("id").eq("id", purchaseIds[2]).single();
        if (!p3Check) throw new Error("FAIL: Unsold purchase in blocked batch was deleted! Atomicity violated.");
        console.log("✓ All-or-nothing rollback verified: unsold purchase in batch remains untouched.");
      } else {
        throw new Error("FAIL: Bulk purchase delete with sold items succeeded when it should have been blocked!");
      }
    } else {
    ...
    const { data: delSaleRes, error: delSaleErr } = await supabase.rpc("pos_delete_sale", {
      p_sale_id: s1Id,
    });
    if (delSaleErr) throw new Error("Failed to delete sale 1: " + delSaleErr.message);
    ...
    const { data: invAfterS1Delete } = await supabase
      ...
      .eq("product_id", product.id)
      .single();
    console.log(`Inventory after S1 delete: ${invAfterS1Delete?.quantity} (expected 20, +5 restored)`);
    ...
    const { data: delSalesBulkRes, error: delSalesBulkErr } = await supabase.rpc("pos_delete_sales_bulk", {
      p_sale_ids: [s2Id],
    });
    if (delSalesBulkErr) throw new Error("Failed to bulk delete sale 2: " + delSalesBulkErr.message);
    ...
    const { data: invAfterS2Delete } = await supabase
      ...
      .eq("product_id", product.id)
      .single();
    console.log(`Inventory after S2 delete: ${invAfterS2Delete?.quantity} (expected 30, all sales deleted)`);
    ...
    const { data: delP1Res, error: delP1Err } = await supabase.rpc("pos_delete_purchase", {
      p_purchase_id: purchaseIds[0],
    });
    if (delP1Err) throw new Error("Failed to delete purchase 1: " + delP1Err.message);
    ...
    const { data: invAfterP1Delete } = await supabase
      ...
      .eq("product_id", product.id)
      .single();
    console.log(`Inventory after P1 delete: ${invAfterP1Delete?.quantity} (expected 20, -10 deducted)`);
    ...
    if (functionsStatus.pos_delete_purchases_bulk) {
      ...
      const { data: delPurBulkRes, error: delPurBulkErr } = await supabase.rpc("pos_delete_purchases_bulk", {
        p_purchase_ids: [purchaseIds[1], purchaseIds[2]],
      });
      if (delPurBulkErr) throw new Error("Failed to bulk delete purchases 2 and 3: " + delPurBulkErr.message);
      ...
      await supabase.rpc("pos_delete_purchase", { p_purchase_id: purchaseIds[1] });
      await supabase.rpc("pos_delete_purchase", { p_purchase_id: purchaseIds[2] });
    ...
    const { data: invAfterBulkPurDelete } = await supabase
      ...
      .eq("product_id", product.id)
      .single();
    console.log(`Inventory after P2 and P3 bulk delete: ${invAfterBulkPurDelete?.quantity} (expected 0)`);
    if (Number(invAfterBulkPurDelete?.quantity) !== 0) {
      throw new Error(`Expected stock 0, got ${invAfterBulkPurDelete?.quantity}`);
    }
