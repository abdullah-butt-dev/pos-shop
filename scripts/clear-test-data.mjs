...
async function clearTestData() {
  ...
  console.log(
    "🔒 Note: auth.users (passwords/usernames) and pos_business_settings are PRESERVED.\n",
  );
  ...
  console.log(
    `pos_sale_cost_allocations: ${scaErr ? "❌ " + scaErr.message : "✅ Cleared " + (scaCount ?? 0) + " rows"}`,
  );
  ...
  const { error: siErr, count: siCount } = await supabase
    ...
    .neq("id", "00000000-0000-0000-0000-000000000000");
  console.log(
    `pos_sale_items: ${siErr ? "❌ " + siErr.message : "✅ Cleared " + (siCount ?? 0) + " rows"}`,
  );
  ...
  const { error: cpErr, count: cpCount } = await supabase
    ...
    .neq("id", "00000000-0000-0000-0000-000000000000");
  console.log(
    `pos_customer_payments: ${cpErr ? "❌ " + cpErr.message : "✅ Cleared " + (cpCount ?? 0) + " rows"}`,
  );
  ...
  const { error: sErr, count: sCount } = await supabase
    ...
    .neq("id", "00000000-0000-0000-0000-000000000000");
  console.log(
    `pos_sales: ${sErr ? "❌ " + sErr.message : "✅ Cleared " + (sCount ?? 0) + " rows"}`,
  );
  ...
  const { error: spErr, count: spCount } = await supabase
    ...
    .neq("id", "00000000-0000-0000-0000-000000000000");
  console.log(
    `pos_supplier_payments: ${spErr ? "❌ " + spErr.message : "✅ Cleared " + (spCount ?? 0) + " rows"}`,
  );
  ...
  if (products && products.length > 0) {
    for (const p of products) {
      await supabase
        .from("pos_inventory")
        .upsert({ product_id: p.id, quantity: 1000000 });
    }
  }
  ...
  const { error: piErr, count: piCount } = await supabase
    ...
    .neq("id", "00000000-0000-0000-0000-000000000000");
  console.log(
    `pos_purchase_items: ${piErr ? "❌ " + piErr.message : "✅ Cleared " + (piCount ?? 0) + " rows"}`,
  );
  ...
  const { error: purErr, count: purCount } = await supabase
    ...
    .neq("id", "00000000-0000-0000-0000-000000000000");
  console.log(
    `pos_purchases: ${purErr ? "❌ " + purErr.message : "✅ Cleared " + (purCount ?? 0) + " rows"}`,
  );
  ...
  const { error: imErr, count: imCount } = await supabase
    ...
    .neq("id", "00000000-0000-0000-0000-000000000000");
  console.log(
    `pos_inventory_movements: ${imErr ? "❌ " + imErr.message : "✅ Cleared " + (imCount ?? 0) + " rows"}`,
  );
  ...
  console.log(
    `pos_inventory: ${invErr ? "❌ " + invErr.message : "✅ Cleared " + (invCount ?? 0) + " rows"}`,
  );
  ...
  const { error: cErr, count: cCount } = await supabase
    ...
    .neq("id", "00000000-0000-0000-0000-000000000000");
  console.log(
    `pos_customers: ${cErr ? "❌ " + cErr.message : "✅ Cleared " + (cCount ?? 0) + " rows"}`,
  );
  ...
  const { error: prodErr, count: prodCount } = await supabase
    ...
    .neq("id", "00000000-0000-0000-0000-000000000000");
  console.log(
    `pos_products: ${prodErr ? "❌ " + prodErr.message : "✅ Cleared " + (prodCount ?? 0) + " rows"}`,
  );
  ...
  const { error: supErr, count: supCount } = await supabase
    ...
    .neq("id", "00000000-0000-0000-0000-000000000000");
  console.log(
    `pos_suppliers: ${supErr ? "❌ " + supErr.message : "✅ Cleared " + (supCount ?? 0) + " rows"}`,
  );
